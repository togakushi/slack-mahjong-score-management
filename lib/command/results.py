...
def __select_game__(argument, command_option):
...
def summary2(argument, command_option):
    ...
    ret = __select_game__(argument, command_option)
    resultdb = sqlite3.connect(g.database_file, detect_types = sqlite3.PARSE_DECLTYPES)
    resultdb.row_factory = sqlite3.Row
    ...
    msg2 += "\t最初のゲーム：{}\n\t最後のゲーム：{}\n".format(
        min([results[name]["first_game"] for name in results.keys()]).replace("-", "/"),
        max([results[name]["last_game"] for name in results.keys()]).replace("-", "/"),
    )
    if ret["target_player"]:
        msg2 += "\t総ゲーム数：{} 回".format(
            int(sum([results[name]["count"] for name in results.keys()])),
        )
    else:
        msg2 += "\tゲーム数：{} 回".format(
            int(sum([results[name]["count"] for name in results.keys()]) / 4),
        )
