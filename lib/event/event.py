...
def handle_message_events(client, body):
    ...
    if re.match(rf"^{g.commandword['help']}$", parameter["text"]):
        ...
        f.slack_api.post_message(
            client, parameter["channel_id"],
            msg, parameter["event_ts"]
        )

        # メンバーリスト
        title, msg = c.member.Getmemberslist()
        f.slack_api.post_text(
            client, parameter["channel_id"], parameter["event_ts"], title, msg
        )
        return
    ...
    if re.match(rf"^{g.commandword['check']}", parameter["text"]):
        d.comparison.main(
            client, parameter["channel_id"], parameter["event_ts"]
        )
        return
    ...
    if msg:
        if not parameter["status"] == "message_deleted" and updatable:
            f.score.check_score(
                client, parameter["channel_id"],
                parameter["event_ts"], parameter["user"], msg
            )
        if updatable:
            match parameter["status"]:
                case "message_append":
            ...
            f.slack_api.post_message(
                client, parameter["channel_id"],
                f.message.restricted_channel(), parameter["event_ts"]
            )
    else:
