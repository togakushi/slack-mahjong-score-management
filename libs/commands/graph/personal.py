...
def plot() -> Tuple[int, str]:
    ...
    df = loader.read_data(os.path.join(g.script_dir, "libs/queries/summary/gamedata.sql"))
...
def statistics_plot() -> Tuple[int, str]:
    ...
    # データ収集
    g.params.update(guest_skip=g.params.get("guest_skip2"))
    df = loader.read_data(os.path.join(g.script_dir, "libs/queries/summary/details.sql"))
