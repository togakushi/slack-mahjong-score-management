...
def main():
    ...
    if len(g.params["player_list"]) == 1:  # 成績レポート
        ...
        if pdf_file:
            slack_message.post_fileupload(f"成績レポート({name})", pdf_file)
        else:
            slack_message.post_message(message.random_reply(message="invalid_argument"))
    elif g.params.get("order"):
        report_file_path = report.winner.plot()
        if report_file_path:
            slack_message.post_fileupload("成績上位者", report_file_path)
        else:
            slack_message.post_message(message.random_reply(message="no_hits"))
    elif g.params.get("statistics"):
        report_file_path = report.monthly.plot()
        if report_file_path:
            slack_message.post_fileupload("月別ゲーム統計", report_file_path)
        else:
            slack_message.post_message(message.random_reply(message="no_hits"))
        ...
        if report_file_path:
            slack_message.post_fileupload("成績一覧", report_file_path)
        else:
            slack_message.post_message(message.random_reply(message="no_hits"))
