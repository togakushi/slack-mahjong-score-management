...
|   キー    |             内容             |                型                | 省略時 |                     備考                      |
| --------- | ---------------------------- | -------------------------------- | ------ | --------------------------------------------- |
| undefined | 未定義の単語の取り扱い       | 数値(`0`or`2`)                   | 0      | typeを指定                                    |
| yakuman   | 役満ワード(メモ機能)         | 文字列(カンマ区切り)             | None   | `word` に `type=0` として登録                 |
| type0     | 役満ワード(メモ機能)         | 文字列(カンマ区切り)             | None   | `word` に `type=0` として登録                 |
| type2     | 個別カウントワード(メモ機能) | 文字列(カンマ区切り)             | None   | `word` に `type=2` として登録                 |
| その他    | 登録される単語               | 数値(追加計算される卓外ポイント) |        | キーを単語として登録</br>ポイントは1000点単位 |
...
| game_results         | 戦績(ゲーム単位の素点と順位)を表示               | 真偽値 | False  | 個人成績/チーム成績専用                        |
| versus_matrix        | 対戦マトリックス表示                             | 真偽値 | False  | 個人成/チーム成績績専用                        |
| individual           | デフォルトの集計対象を切り替え                   | 真偽値 | True   | `True` : 個人成績<br />`False` : チーム成績    |
| statistics           | 「統計」オプションを常に指定                     | 真偽値 | False  | `results` で座席データ、レコードを常に表示する |
