...
定義されているキーが `words` テーブルの `word` として事前登録される。</br>
...
| その他    | `type=1`として登録される事前登録ワード | 数値(追加計算される卓外ポイント) |        | キーを単語として登録</br>ポイントは1000点単位  |
...
| score_comparisons    | 比較モードで表示する                             | 真偽値 | False  |         |       |         |        | 内部フラグ                                  |
| game_results         | 戦績(ゲーム単位の素点と順位)を表示               | 真偽値 | False  |         |       |         |        | 内部フラグ                                  |
