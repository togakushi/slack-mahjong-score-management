...
| ignore_flying | 箱下のカウント表示 | 真偽値                          | *必須*  |                                                |
...
| reaction_ok       | 素点合計が正しい場合に付けるリアクション     | 文字列(絵文字) | ok             | ゲーム結果のポストに対して付く                  |
...
| after   | 検索開始日                       | 文字列(日付)         | None   | 指定時は検索クエリに追加される |
...
| team_registration_limit | 登録チーム数上限                   | 数値   | 8      |      |
| team_character_limit    | 登録チーム名文字数上限             | 数値   | 16     |      |
