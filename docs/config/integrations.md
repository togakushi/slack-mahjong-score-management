...
`slack_<チャンネルID>`セクションがあればそのチャンネル専用となる。\
メイン設定のslackセクションの設定を上書きされ、チャンネル個別設定の値となる。
...
| channel_config | 個別設定ファイル                             | 文字列(ファイルパス) | None     | [メイン設定](./mainconf.md)とマージ |

> [!TIP]
> `channel_config`で指定した設定ファイルに`mahjong`、`setting`、`results`、`graph`、`ranking`、`report`のセクションを設定する。\
> 各セクションで設定された値は以下の順でマージされるため、未定義の設定はメイン設定の値を引き継ぐ。
> 1. メイン設定セクション読み込み
> 1. チャンネル個別設定セクション読み込み
> \
> `mahjongセクション`の`rule_config`が各コマンドに対してのルールバージョン省略時の規定値となる。\
> チャンネル個別設定に設定された`mahjongセクション`のルールセットはアプリ起動時に取り込まれないため、意味を持たない。

> [!IMPORTANT]
> メイン設定の`commandword`、`help`で指定されているキーワードは上書きできない。

> [!TIP]
> メイン設定に[`custom_message`](./mainconf.md#メッセージカスタマイズオプション)が設定されていれば、そのの内容は無視される。

> [!WARNING]
> settingセクションの`database_file`が再定義できるため、別のDBファイルに情報を蓄積することが可能となる。\
