...
slackセクションに以下のキーがあればメイン設定の値を上書きする。

> [!NOTE]
> `channel_config`はチャンネル個別設定専用パラメータ
...
> [!TIP]
> `channel_config`で指定した設定ファイルに`setting`、`results`、`graph`、`ranking`、`report`、`custom_message`のセクションを設定する。\
