...
`custom_messageセクション`が定義されていればメイン設定の内容を上書き、未定義時はメイン設定の値が引き継がれる。
...
| ------ | ------------ | --------------------------------------------------------------------------------------- | ---------------------------------------------------------- |
| 1      | メイン設定   | チャンネル個別設定<br />[slack](#チャンネル個別設定) / [discord](#チャンネル個別設定-1) | 指定チャンネルの範囲                                       |
| 2      | メイン設定   | [共通設定](#共通設定)                                                                   | 連携サービスの範囲                                         |
| 3      | メイン設定   | [setting](mainconf.md#settingセクション)                                                | アプリケーション全体(すべてのサービス、すべてのチャンネル) |

> [!TIP]
> `channel_config`で指定している個別設定ファイルのsettingセクションでも有効な設定として定義可能だが、探索範囲に含まれないため値は採用されない。
...
### チャンネル個別設定
メイン設定内に `slack_<チャンネルID>` [^1] セクションが存在すれば、そのチャンネル専用として追加で設定を読み込む。\
...
### チャンネル個別設定
メイン設定内に `discord_<チャンネルID>セクション` [^1] が存在すれば、そのチャンネル専用として追加で設定を読み込む。\
...
| custom_css        | カスタマイズスタイルシート[^2]    | 文字列(ファイルパス) | 空欄      | ファイルが存在しない場合は空欄になる                                                      |
