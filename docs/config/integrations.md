...
| ------ | ------------ | ----------------------------------------- | ---------------------------------------------------------- |
| 1      | メイン設定   | [チャンネル個別設定](#チャンネル個別設定) | 指定チャンネル単位                                         |
| 2      | メイン設定   | [共通設定](#共通設定)                     | 連携サービスの範囲                                         |
| 3      | メイン設定   | [setting](mainconf.md#settingセクション)  | アプリケーション全体(すべてのサービス、すべてのチャンネル) |

> [!TIP]
> `channel_config`で指定している個別設定ファイルのsettingセクションでも有効な設定として設定可能だが、探索範囲に含まれないため値は採用されない。
...
slackセクションに以下のキーが存在すればメイン設定の値を上書きする。
