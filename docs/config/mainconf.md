...
> [!NOTE]
> mahjongセクションの`rule_version`が集計基準のルールセット(未指定時のデフォルト)となる。

## 卓外清算(レギュレーション)
### regulationsセクション
定義されているキーが [wordsテーブル](../development/datedase/schema.md#typeの種別)の`word`として事前登録される。\
...
> [!IMPORTANT]
> `files/default_rule.ini`には ***default_rule*** (四人打ち用ルールセット)と ***default_rule3*** (三人打ち用ルールセット)が定義されている。\
...
|        キー        |        内容        |               型               |                   未定義時                    |                        備考                        |
| ------------------ | ------------------ | ------------------------------ | --------------------------------------------- | -------------------------------------------------- |
| <任意のキーワード> | 成績記録キーワード | 文字列(ルールバージョン識別子) | 値の省略時はmahjongセクションの`rule_version` | 定義済みの`rule_version`がないマッピングは無視する |
...
|         キー         |                       内容                       |   型   | 未定義時 | results | graph | ranking | report |                         備考                         |
| -------------------- | ------------------------------------------------ | ------ | -------- | :-----: | :---: | :-----: | :----: | ---------------------------------------------------- |
