...
| ゲーム                                               | 半荘など精算の区切りの単位                                                                                     |
| 突合<br />突合処理                                   | Slack / Discord 上の入力情報を読み込み直し、DBの登録状況を最新化する [^1] [^2]                                 |
| ルールセット                                         | 集計に影響のあるルール(配給原点、返し点、順位点、など)をパラメータ化してまとめたもの<br />集計の基本単位となる |
| ルールバージョン<br />ルールバージョン識別子         | ルールセットに付けた名称                                                                                       |

[^1]: 突合範囲は[サービス個別設定](config/integrations.md)の`search_after`で指定する。
[^2]: DBに記録済みのデータが Slack / Discord で見つからない場合は不要な記録としてDBから削除される。
