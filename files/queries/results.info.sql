...
select distinct
    ...
    ifnull(sum(score) over (partition by name, id), 0) as score,
    ifnull(sum(case when rank = 1 then score else null end) over (partition by name, id), 0) as score_rank1,
    ifnull(sum(case when rank = 2 then score else null end) over (partition by name, id), 0) as score_rank2,
    ifnull(sum(case when rank = 3 then score else null end) over (partition by name, id), 0) as score_rank3,
